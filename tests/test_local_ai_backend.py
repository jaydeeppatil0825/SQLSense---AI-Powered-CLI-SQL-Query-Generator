"""Tests for local Ollama backend calls."""
...
def test_call_ollama_uses_local_url_and_timeout(monkeypatch):
    ...
    with patch("ai.sql_generator.requests.post", return_value=response) as mock_post:
...
def test_call_ollama_timeout_has_clean_message(monkeypatch):
    ...
    with patch("ai.sql_generator.requests.post", side_effect=pytest.importorskip("requests").exceptions.Timeout):
